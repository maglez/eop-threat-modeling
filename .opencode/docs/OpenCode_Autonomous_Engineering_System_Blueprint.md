...
> **Verification does not stop at the merge (ADR-072, 2026-09-07).** The Playwright end-to-end tier is too slow to gate a pull request — three browsers run serially because there is one app instance, one database and two rate limiters keyed on the runner's own address — so it runs *after* the merge instead: on every push to `main`, nightly at 06:00 UTC, and on demand. It is not merely absent from branch protection, it never runs on a `pull_request` event at all, so it cannot be made a required check without a visible edit to its trigger. A failed `image` job **skips** it rather than failing it, which is honest — the suite never got an artefact to run against. Failures reach a human through GitHub's own failed-run notification, with no email action and no secret, alongside a run-summary table naming each scenario that did not pass first time. Two residuals are carried rather than closed: the notification reaches whoever *merged*, who under the human-merge rule is the reviewing engineer rather than the agent that opened the PR; and because this is a detector and not a gate, a regression can sit on `main` for the length of one run.
