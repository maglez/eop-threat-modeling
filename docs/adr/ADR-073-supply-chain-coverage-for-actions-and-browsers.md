...
```
mcr.microsoft.com/playwright:v1.63.0-noble@sha256:eff16c30…
```

invoked from the runner as a single `docker run` (`--rm --network host --ipc host -u "$(id -u):$(id -g)" -e HOME=/tmp -e E2E_REUSE_STACK --mount type=bind,source="$(pwd)",target=/e2e --workdir /e2e`). The digest appears in exactly one *operative* place, `.github/workflows/ci.yml`, mirrored only by the `tools/supply-chain/expected-containers.json` entry that audits it. Note this document, `CHANGELOG.md` and `docs/adr/README.md` deliberately quote it **truncated**, and that is not a stylistic choice: `audit-containers.sh` treats every tracked file carrying a pinned reference as an `occurrences` entry it must be told about, bidirectionally, so spelling the full 64 hex characters here would either fail the audit or force this dated record to be rewritten the next time the pin moves. The first draft of this change did spell it in full in all three files, and the audit caught it — `mcr.microsoft.com/playwright now appears in ['CHANGELOG.md', 'docs/adr/ADR-073-supply-chain-coverage-for-actions-and-browsers.md', 'docs/adr/README.md'], which the baseline does not list. Every place a digest is written is a place it can go stale -- list it, or remove the duplication.` The duplication was removed rather than declared, because prose that has to be edited to move a pin is prose that will be forgotten.
